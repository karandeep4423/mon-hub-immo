...
import { useAdminCollaborations, type Collaboration } from '@/hooks/useAdminCollaborations';
...
export default function AdminCollaborationsPage() {
	...
	const allowedStatuses = ['pending', 'active', 'completed', 'cancelled'] as const;
	type AllowedStatus = typeof allowedStatuses[number];

	const normalized: AdminCollaboration[] | undefined = collaborations?.map((c: Collaboration) => {
	...
	return (
		<AdminLayout>
			<AdminCollaborationsTableModern collaborations={normalized} loading={loading} />
		</AdminLayout>
	);
}
