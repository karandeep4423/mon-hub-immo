...
const getCollaborationType = (
	...
	if (ownerType === 'agent' && collabType === 'apporteur')
		return 'agent-apporteur';
	if (ownerType === 'apporteur' && collabType === 'agent')
		return 'agent-apporteur';
		...
		<div className="space-y-3 sm:space-y-4 lg:space-y-6">
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
				<StatCard
					...
					label="Total"
					value={filteredCollaborations.length}
					color="blue"
				/>
				<StatCard
					icon={
				...
				<StatCard
					...
					label="Commissions"
					value={`€${(filteredCollaborations.reduce((sum, c) => sum + (c.commission || c.proposedCommission || 0), 0) / 1000).toFixed(1)}k`}
					color="rose"
				/>
			</div>
					...
					columns={[
						{
							...
							render: (
								...
								const apporteurName =
									row.apporteurName ||
									(typeof row.apporteur === 'string'
										? row.apporteur
										: row.apporteur
											? `${row.apporteur.firstName ?? ''} ${row.apporteur.lastName ?? ''}`.trim()
											: '');
								const apporteurId =
									typeof row.apporteur === 'object' &&
									row.apporteur
										? row.apporteur._id
										: row.apporteurId;
								return (
...
const StatCard: React.FC<{
	...
	return (
		<div
			className={`bg-gradient-to-br ${colors[color as keyof typeof colors]} border rounded-lg p-2.5 sm:p-3 lg:p-4`}
		>
