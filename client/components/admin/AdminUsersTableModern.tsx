...
export const AdminUsersTableModern: React.FC<AdminUsersTableModernProps> = ({
	...
	const [filters, setFilters] = useState<{
		...
		network: string;
		email: string;
	}>({ type: '', status: '', search: '', network: '', email: '' });
	const [editingUser, setEditingUser] = useState<AdminUser | null>(null);
	const [showCreate, setShowCreate] = useState(false);
	const [showImport, setShowImport] = useState(false);
	const [tableConfirmAction, setTableConfirmAction] = useState<null | {
		label: string;
	...
	const filteredUsers = useMemo<AdminUser[]>(() => {
		...
		const usersToFilter = (users || []).map((u: AdminUser) => {
			...
			const status = ((uu['status'] as string) ||
				((uu['isBlocked'] as boolean)
					? 'blocked'
					: (uu['isValidated'] as boolean)
						? 'active'
						: 'pending')) as 'active' | 'pending' | 'blocked';
			const registeredAt = ((uu['registeredAt'] as string) ||
				(uu['createdAt'] as string) ||
				(uu['createdAtISO'] as string) ||
				(uu['created_at'] as string) ||
				'') as string;
			const type = ((uu['type'] as string) ||
				(uu['userType'] as string) ||
				(uu['role'] as string) ||
				'apporteur') as 'agent' | 'apporteur' | 'admin';
			return {
				...u,
				status,
				registeredAt,
	...
	const exportToXLS = (usersToExport: AdminUser[] = filteredUsers) => {
		...
		const rows = usersToExport.map((u) => [
			...
			Number(
				(u as unknown as Record<string, unknown>)['propertiesCount'],
			) || 0,
			(Number(
			...
			Number(
				(u as unknown as Record<string, unknown>)['connectionsCount'],
			) || 0,
					...
					variant={
						tableConfirmAction.type === 'block'
							...
							: tableConfirmAction.type === 'unblock'
								? 'primary'
								: tableConfirmAction.type?.includes('manual')
									? 'warning'
									: 'primary'
					}
					loading={actionBusy}
							...
							render: (_: unknown, row: AdminUser) => (
								<div className="flex items-center gap-2 min-w-0">
									<div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex-shrink-0 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-md">
										{row.firstName && row.firstName.charAt
									...
									<button
										...
										onClick={() =>
											setTableConfirmAction({
												label: 'Débloquer cet utilisateur ?',
												type: 'unblock',
												onConfirm: async () => {
													setActionBusy(true);
													try {
								...
								<div className="hidden lg:flex">
									{row.accessGrantedByAdmin ? (
										<button
											...
											onClick={() =>
												setTableConfirmAction({
													label: "Révoquer l'accès manuel pour cet utilisateur ?",
													type: 'revoke_manual',
													onConfirm: async () => {
														setActionBusy(true);
														try {
															await adminService.revokeAdminAccess(
																row._id,
															);
															await refetch();
															toast.info(
																'Accès manuel révoqué.',
															);
														} catch (err) {
															console.error(err);
															toast.error(
										...
										<button
											...
											onClick={() =>
												setTableConfirmAction({
													label: "Donner l'accès manuel à cet utilisateur (outrepasse le paiement) ?",
													type: 'grant_manual',
													onConfirm: async () => {
														setActionBusy(true);
														try {
															await adminService.grantAdminAccess(
																row._id,
															);
															await refetch();
															toast.success(
																'Accès manuel accordé.',
															);
														} catch (err: unknown) {
															console.error(err);
															const msg =
										...
										onClick={() =>
											setTableConfirmAction({
												...
												onConfirm: async () => {
													setActionBusy(true);
													try {
														await adminService.sendPaymentReminder(
															row._id,
														);
														toast.success(
...
const FilterStatCard: React.FC<{
	...
	return (
		<div
			className={`bg-gradient-to-br ${colors[color as keyof typeof colors]} border rounded-lg p-3 sm:p-4`}
		>
...
const EditUserModal: React.FC<{
	...
	const [confirmAction, setConfirmAction] = useState<null | {
		type: 'block' | 'validate' | 'manualAccess';
		label: string;
		onConfirm: () => void;
	}>(null);
						...
						<div className="flex items-start gap-6">
							...
							<div className="flex-1">
								...
								<div className="mt-3 flex flex-wrap gap-2">
									...
									>{`Validé: ${form.isValidated ? 'Oui' : 'Non'}`}</Badge>
								</div>
							</div>
						</div>
