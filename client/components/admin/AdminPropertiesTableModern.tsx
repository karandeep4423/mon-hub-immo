 		...
 		<div className="space-y-6">
 			<div className="flex justify-between items-center">
 				...
 				<div className="flex gap-2">
 					<Button variant="secondary" size="md" onClick={() => exportAsCSV('csv')}>⬇️ Export CSV</Button>
 					<Button variant="secondary" size="md" onClick={() => exportAsCSV('xls')}>⬇️ Export XLS</Button>
 					<Button variant="primary" size="md">➕ Nouvelle Annonce</Button>
 				</div>
 			</div>
 			...
 			<div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
 				...
 				<div className="flex border border-gray-300 rounded-lg bg-gray-50">
 					<button onClick={() => setViewType('table')} className={`px-3 py-2 transition-colors ${viewType === 'table' ? 'bg-cyan-500 text-white' : 'text-gray-700 hover:bg-gray-100'}`}>📋</button>
 					<button onClick={() => setViewType('grid')} className={`px-3 py-2 transition-colors ${viewType === 'grid' ? 'bg-cyan-500 text-white' : 'text-gray-700 hover:bg-gray-100'}`}>📊</button>
 				</div>
 			</div>

 			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
 				<div className="bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-100 border rounded-lg p-4"><div><p className="text-sm text-gray-600">Total</p><p className="text-2xl font-bold text-gray-900 mt-1">{totalItems ?? properties.length}</p></div><span className="text-3xl">🏠</span></div>
 				<div className="bg-gradient-to-br from-emerald-50 to-green-50 border-emerald-100 border rounded-lg p-4"><div><p className="text-sm text-gray-600">Actives</p><p className="text-2xl font-bold text-gray-900 mt-1">{properties.filter((p:any) => p.status === 'active').length}</p></div><span className="text-3xl">✅</span></div>
 				<div className="bg-gradient-to-br from-purple-50 to-indigo-50 border-purple-100 border rounded-lg p-4"><div><p className="text-sm text-gray-600">Vues</p><p className="text-2xl font-bold text-gray-900 mt-1">{properties.reduce((sum:number, p:any) => sum + (p.views || 0), 0)}</p></div><span className="text-3xl">📊</span></div>
 				<div className="bg-gradient-to-br from-rose-50 to-pink-50 border-rose-100 border rounded-lg p-4"><div><p className="text-sm text-gray-600">Valeur Total</p><p className="text-2xl font-bold text-gray-900 mt-1">€{(properties.reduce((sum:number, p:any) => sum + (p.price || 0), 0) / 1000000).toFixed(1)}M</p></div><span className="text-3xl">💰</span></div>
 			</div>
 				...
 				<DataTable
 					...
 					actions={(row: any) => (
 						<div className="flex items-center gap-2">
 							<Link href={`/property/${row._id}`} className="p-1 hover:bg-blue-100 rounded transition-colors" title="Voir">👁️</Link>
 							<button className="p-1 hover:bg-yellow-100 rounded transition-colors" title="Éditer">✏️</button>
 							<button className="p-1 hover:bg-red-100 rounded transition-colors" title="Supprimer" onClick={() => handleDelete(row._id)}>🗑️</button>
 						</div>
 					)}
 				/>
 			)}
 				...
 				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
 					{properties.map((prop: any) => (
 						<div key={prop._id} className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow group">
 							<div className="w-full h-40 bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center text-4xl group-hover:scale-105 transition-transform">🏠</div>
 							<div className="p-4">
 								<h3 className="font-bold text-gray-900 mb-1">{prop.title}</h3>
 								<p className="text-xs text-gray-500 mb-3">📍 {prop.location || prop.city}</p>
