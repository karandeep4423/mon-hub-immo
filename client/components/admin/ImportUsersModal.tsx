				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div>
						<label className="block text-sm font-medium mb-2">
							Fichier CSV (colonnes: email, firstName, lastName,
							userType, phone, network)
						</label>
						<input
							type="file"
							accept=".csv"
						...
						<p className="text-xs text-gray-500 mt-1">
							Format CSV requis. Colonnes obligatoires: email,
							firstName, lastName
						</p>
					</div>
					...
					<div className="flex items-center gap-4">
						<label className="inline-flex items-center gap-2">
							...
							<span className="text-sm">
								Envoyer une invitation (lien de définition du
								mot de passe)
							</span>
						</label>
						...
						<label className="inline-flex items-center gap-2">
							...
							/>
							<span className="text-sm">
								Valider automatiquement les comptes
							</span>
						</label>
					</div>
					...
					<div className="flex items-center gap-2">
						...
						<select
							...
							onChange={(
								...
								setDefaultUserType(
									e.target.value as 'agent' | 'apporteur',
								)
							}
					...
					<div className="text-sm mt-2">
						<label className="inline-flex items-center gap-2">
							...
							/>
							<span className="ml-1">
								Mettre à jour les utilisateurs existants si
								l&apos;email existe (updateIfExists)
							</span>
						</label>
					</div>
				...
				<div className="space-y-4">
					<div className="p-4 bg-green-50 border border-green-200 rounded">
						...
						<p className="text-sm">
							...
							{result.errors ? result.errors.length : 0} erreur(s)
							{result.updatedCount
						...
						<div>
							...
							<div className="space-y-1 text-sm max-h-40 overflow-y-auto">
								{result.created.map((u) => (
									<p key={u.id} className="text-gray-700">
										✓ {u.email}
									</p>
								))}
							</div>
