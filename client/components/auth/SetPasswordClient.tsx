...
export default function SetPasswordClient() {
  ...
  const { values, errors, handleInputChange, setErrors, isSubmitting, handleSubmit } = useForm({
    ...
    onSubmit: async (data: unknown) => {
      try {
        const payloadData = data as { token?: string; newPassword?: string; confirmPassword?: string };
        ...
        setPasswordSchema.parse(payload as { email: string; token: string; newPassword: string });
