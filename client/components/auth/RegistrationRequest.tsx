...
export const RegistrationRequest: React.FC = () => {
	...
	const [formData, setFormData] = useState({
		...
		email: '',
		phone: '',
		userType: 'apporteur' as 'agent' | 'apporteur',
		password: '',
		confirmPassword: '',
	});
	...
	const validateForm = () => {
		...
		if (!formData.confirmPassword.trim())
			newErrors.confirmPassword = 'Confirmation requis';
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response.success) {
				...
				router.push(
					`/auth/verify-email?email=${encodeURIComponent(formData.email)}&redirect=profile`,
				);
			} else {
