		...
		onSubmit: async (data) => {
			...
			try {
				...
				if (response.success && response.user) {
					...
					logger.debug(
						'[VerifyEmailForm] Checking profile completion',
						{
							requiresProfileCompletion:
								response.requiresProfileCompletion,
							willRedirectTo: response.requiresProfileCompletion
								? Features.Auth.AUTH_ROUTES.COMPLETE_PROFILE
								: Features.Auth.AUTH_ROUTES.WELCOME,
						},
					);
			...
			<form onSubmit={handleSubmit} className="space-y-6">
				...
				<div className="space-y-2">
					<Input
						...
						error={errors.code}
						placeholder={
							Features.Auth.AUTH_PLACEHOLDERS.VERIFICATION_CODE
						}
						maxLength={8}
				...
				{process.env.NODE_ENV === 'development' && (
					<div className="text-xs text-gray-500 text-center">
						Code length: {values.code.length} | Value: {values.code}
					</div>
				)}
			</form>
