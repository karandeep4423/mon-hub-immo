...
router.get('/conversation', (async (req, res) => {
	try {
		...
		const collaboration = await Collaboration.findById(collaborationId).select('postOwnerId collaboratorId status');
		if (!collaboration) {
...
router.get('/messages', (async (req, res) => {
	try {
		const { userA, userB, before, limit } = req.query as { userA?: string; userB?: string; before?: string; limit?: string };
		if (!userA || !userB || !Types.ObjectId.isValid(userA) || !Types.ObjectId.isValid(userB)) {
			return res.status(400).json({ error: 'Paramètres userA/userB invalides' });
		}
		...
		const results = await Message.find(query).sort({ createdAt: -1 }).limit(take);
		return res.status(200).json({ messages: results.reverse(), count: results.length });
	} catch (e) {
